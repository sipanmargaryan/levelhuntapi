...
class SignupSerializer(serializers.ModelSerializer):
    ...
    def generate_username(self, first_name, last_name):
        username = f'{first_name}_{last_name[0]}_{uuid.uuid4().hex[:8]}'.lower()
        if User.objects.filter(username=username).exists():
            self.generate_username(first_name, last_name)  # pragma: no cover
    ...
    def create(self, validated_data):
        ...
        if user:
            if user.can_request_password_reset():
                ...
                raise serializers.ValidationError(_('From your last request does not pass 5 minutes.'))
...
class ResetPasswordSerializer(serializers.Serializer, AuthPayload):
    ...
    def validate_token(self, value):
        ...
        elif not self.user.can_reset_password():
            raise serializers.ValidationError(_('Reset password link has been expired.'))
    ...
    def create(self, validated_data):
        ...
        users.utils.emails.send_change_password(self.user)
...
class SocialConnectSerializer(serializers.ModelSerializer, AuthPayload):
    ...
    def create(self, validated_data):
        ...
        social = SocialConnection.objects.filter(
        ...
        if social is None:
            ...
            social = SocialConnection.objects.create(
                provider_id=provider_id,
                provider=provider,
                user=user,
            )

        return self.get_auth_payload(user=social.user)
